...
def ApptDataView(request, pk):
	...
	if request.method=='Post':
		form = PatientApptForm(request.POST or None, instance=appt)
		appt = form.save(commit=False)
		appt.patient = patient
		appt.user = patient
		appt.current_health_conditions = patient_conditions
		appt.save()
		return HttpResponseRedirect('formsuccess')
	else:
...
def ApptDataEdit(request, pk):
	...
	if request.method =="POST":
		...
		form = PatientApptForm(request.POST, instance=appt)
		if form.is_valid():
			form.save()
	...
	return render(request, 'view_appts_edit.html', context)
